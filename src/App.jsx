...
const enviarInformeDiarioEmailJS = async (params = {}) => {
  ...
  if (config) {
    ...
    const vendidosEnPeriodo = laptops.filter(lap => lap.estado?.toUpperCase().trim() === 'VENDIDO' && esDelPeriodo(lap.fecha_venta || lap.fecha, tipo, fecha));
 ...
 const guardarLaptop = async (e) => {
  ...
  if (!editandoId) {
    // El usuario quiere registrar tantas máquinas como la cantidad, incluso sin series.
    // `listaSeriales` ya tiene la longitud correcta basada en `form.cantidad` desde el formulario.

    // 1. Obtenemos los seriales ingresados, manteniendo los espacios vacíos.
    const serialesIngresados = listaSeriales.map(s => s.trim().toUpperCase());

    // 2. Verificamos duplicados SOLO entre los seriales que sí se rellenaron.
    const serialesRellenos = serialesIngresados.filter(s => s !== "");
    const tieneDuplicados = new Set(serialesRellenos).size !== serialesRellenos.length;
    if (tieneDuplicados) {
      alert("❌ Hay seriales duplicados en la lista.");
      return; // Detenemos la ejecución.
    }
    serialesFinales = serialesIngresados;
  } else {
    const serialEditado = listaSeriales[0]?.trim().toUpperCase();
    // --- CORRECCIÓN DE BUG (NIVEL 2) ---
    // El número de serie solo es estrictamente obligatorio si el equipo se está marcando como 'VENDIDO'.
    // Para 'STOCK' o 'PENDIENTE', puede guardarse sin serial.
    if (form.estado === 'VENDIDO' && !serialEditado) {
      return alert("❌ Para marcar un equipo como VENDIDO, se requiere un número de serie.");
    }
    serialesFinales = [serialEditado || '']; // Guardamos el serial o un string vacío si no se proveyó.
  }
...
const laptopsFiltradas = laptops.filter(lap => {
  ...
  // ✅ FIX: Normalizamos el responsable (puede venir como string u objeto, con
  // mayúsculas/minúsculas distintas: "Ray", "RAY", "ray"...). Comparamos sin
  // distinguir mayúsculas y revisando tanto 'responsable' como 'vendedor',
  // para que Ray (Tienda Principal) siempre vea los registros que le corresponden.
  const campoResponsable = lap.responsable || lap.vendedor;
  const responsableEquipo = String(
    (typeof campoResponsable === 'object' && campoResponsable !== null)
      ? campoResponsable.nombre
      : campoResponsable
  ).trim().toUpperCase();
  const nombreUsuario = String(usuarioLogueado?.nombre || '').trim().toUpperCase();

  const coincideVendedor = puedeVerTodo
    ? true
    : (responsableEquipo === nombreUsuario);
...
// --- Lógica para el menú de navegación desplazable ---
  const navItems = useMemo(() => {
    ...
    if (tienePermiso(PERMISSIONS.VER_VENTAS)) {
      ...
      items.push({ pestana: 'despachos', texto: '🚚 SALIDAS', style: { border: '1px solid rgba(59, 130, 246, 0.5)', whiteSpace: 'nowrap', '--color-activo': '#3b82f6' } });
    }
    ...
    if (tienePermiso(PERMISSIONS.GESTIONAR_USUARIOS)) {
      items.push({ pestana: 'gestion_usuarios', texto: '👤 USUARIOS', style: { border: '1px solid rgba(255, 105, 180, 0.6)', whiteSpace: 'nowrap', '--color-activo': '#ec4899' } });
    }
      ...
      <div id="login-root">
        {!mostrarPin ? (
          <form className="login-box" onSubmit={(e) => { 
            e.preventDefault();
            let usuarioInput = userDigitado.toLowerCase().trim();
            const passInput = passDigitado.trim();

            // CORRECCIÓN: Permite que el usuario "ray" inicie sesión con su nombre en lugar de "vendedor".
            if (usuarioInput === 'ray') {
              usuarioInput = 'vendedor';
            }
      ...
      <main className="main-content">
        ...
        {pestanaActual === 'excel_interno' && (
          <div className="fade-in table-wrapper-global">
            ...
            <HojaReportes 
              ...
              usuarioLogueado={usuarioLogueado}
              activarEdicion={activarEdicion}
              setModalImagen={(img) => { setModalImagen(img); setFotoActual(Array.isArray(img) ? img[0] : img); }}
        ...
        {pestanaActual === 'despachos' && (
          <DespachosView 
            laptops={laptops}
            usuarioLogueado={usuarioLogueado}
            iniciarEscaneo={iniciarEscaneo} // Pasar la función iniciarEscaneo
          />
        )}
      </main>
    </div>
