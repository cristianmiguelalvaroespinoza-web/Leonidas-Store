...
// 1. Agregamos onVenderClick y modoVentas a las props
const AlmacenTabla = ({ 
  usuarioLogueado, 
  activarEdicion, 
  laptops, 
  ...
  const descargarExcel = () => {
    const tablaExcel = laptopsFiltradas.map((l, i) => {
      // Definimos el orden de las columnas básicas
      const fila = {
        'N°': i + 1,
        FECHA: l.fecha,
      ...
      // Si es super_admin, inyectamos COSTO antes de PRECIO
      ...
      // Si es super_admin, inyectamos UTILIDAD después de PRECIO
      if (usuarioLogueado?.rol === 'super_admin') {
  ...
  return (
    <div className="excel-container fade-in">
      <div className="excel-header-actions" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <h2 style={{color: '#fff', marginBottom: '1rem'}}>📊 {modoVentas ? "GESTIÓN DE VENTAS" : "TABLA DE INVENTARIO"}</h2>
        <button 
                  ...
                  <div className="excel-actions-btns" style={{display: 'flex', flexDirection: 'row', justifyContent: 'center', gap: '8px'}}>
                    {modoVentas && l.estado !== 'VENDIDO' && (
                      <button 
                        ...
                        title="Realizar Venta de este equipo"
                      >
                        💰
                      </button>
                    )}
                    <button 
                      ...
                      onClick={() => (l.imagenes && l.imagenes.length > 0) ? setModalImagen(l.imagenes) : alert("Sin imágenes")}
