...
{/* BOTÓN IR A DESPACHOS */}
          {!modoVentas && (
            <button 
              className="btn-excel-download" 
              onClick={() => setVistaActual(vistaActual === 'inventario' ? 'despachos' : 'inventario')}
              style={{ 
                height: '36px', display: 'flex', alignItems: 'center', marginRight: '10px',
                background: vistaActual === 'despachos' ? '#3b82f6' : 'rgba(59, 130, 246, 0.1)',
                color: vistaActual === 'despachos' ? '#fff' : '#3b82f6',
                border: '1px solid #3b82f6'
              }}
            >
              {vistaActual === 'inventario' ? '🚚 IR A DESPACHOS' : '📦 VER INVENTARIO'}
            </button>
          )}
...
{vistaActual === 'inventario' ? (
  ...
  {(() => {
    ...
    return laptopsOrdenadas.map((l, i) => {
     ...
     <div className="contenedor-despachos" style={{ marginTop: '10px' }}>
         <DespachosView laptops={laptops} usuarioLogueado={usuarioLogueado} />
      </div>
      )} 
    </>
  );
