...
const AlmacenTabla = ({ 
    ...
    try {
      ...
      laptops.forEach((l, i) => {
        ...
        const rowData = {
          ...
          marca: l.marca,
          modelo: l.modelo,
          procesador: `${l.procesador || ''} ${corregirGeneracion(l.generacion || l.gen)}`.trim() || 'N/A',
          ram: l.ram || 'N/A',
          gpu: l.gpu || 'N/A',
        ...
        if (usuarioLogueado?.rol === 'super_admin') {
          ...
          // ✅ CORRECCIÓN: Si no está vendido, la utilidad en el reporte es 0
          rowData.utilidad = esVendido ? (l.utilidad || (Number(l.precio) - Number(l.precio_costo)) || 0) : 0;
        }
  ...
  // 👇 FUNCIÓN DE INTERPRETACIÓN DE FECHAS 👇
        ...
        <table className="excel-table" style={{ width: '100%', borderCollapse: 'collapse', backgroundColor: '#0f172a' }}>
          <thead>
            <tr style={{ backgroundColor: '#1e293b', borderBottom: '2px solid #334155' }}>
              ...
              {usuarioLogueado?.rol === 'super_admin' && (
                <th style={{ ...estiloCelda, fontSize: '11px', textAlign: 'left', color: '#00ff7f', fontWeight: '600' }}>GANANCIA</th>
              )}
          ...
          <tbody>
            {(() => {
              ...
              return laptops.map((l, i) => {
                ...
                const esVendido = l.estado?.toLowerCase() === 'vendido';
                    ...
                    {estaExpandido && (
                      <tr className="row-hover-simple" style={{ backgroundColor: 'transparent' }}>
                        ...
                        <td style={{ ...estiloCelda, color: '#60a5fa' }}>
                          {nombreVendedorVisible(typeof (l.responsable || l.vendedor) === 'object'
                        ...
                        <td style={{ ...estiloCelda, textAlign: 'center', color: '#fff' }}>
                          {l.procesador || '-'} {corregirGeneracion(l.generacion || l.gen) || ''}
                        </td>
                        ...
                        {usuarioLogueado?.rol === 'super_admin' && (
                          <td style={{ ...estiloCelda, color: '#00ff7f', fontWeight: 'bold' }}>
                            {/* ✅ CORRECCIÓN VISUAL: Solo calcula utilidad en la tabla si ya está vendida */}
                            S/ {esVendido ? (l.utilidad || (Number(l.precio) - Number(l.precio_costo)) || 0) : 0}
                          </td>
                        )}
                        <td style={estiloCelda}>
                          <span style={{ 
                            fontSize: '10px',
                            color: esVendido ? '#ef4444' : (l.estado?.toUpperCase() === 'PENDIENTE' ? '#f59e0b' : '#00ff7f'),
