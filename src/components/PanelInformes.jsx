  ...
  const descargarExcelEstiloOscuro = async () => {
    ...
    datosParaExportar.forEach((lap, index) => {
      const row = worksheet.addRow({
        index: index + 1,
        fecha: lap.fecha || 'N/A',
        vendedor: typeof lap.vendedor === 'object' ? lap.vendedor?.nombre : (lap.vendedor || 'Personal administrativo'),
        marca: lap.marca || 'N/A',
...
// FUNCIÓN PARA CONTROLAR EL MODAL DE LEONIDAS
  ...
  const abrirDetallesVendedor = (nombreVendedor) => {
    const filtradas = registrosParaTabla.filter(l => {
      ...
      else if (nombreEnDB.includes('RAY')) asignado = 'RAY';
        else if (nombreEnDB.includes('PIA')) asignado = 'JEFE'; // PIA cuenta como GENERAL
  ...
  const generarExcelVendedor = (nombre, cantidad, total) => {
    try {
      ...
      // --- INICIO DE ESTILOS FINPRO ---
    // 1. Definimos la paleta de colores
    const estiloOscuro = { 
      font: { bold: true, color: { rgb: "FFFFFF" } }, 
      fill: { fgColor: { rgb: "0F172A" } } // Azul oscuro elegante
    };
    const estiloVerde = { 
      font: { bold: true, color: { rgb: "FFFFFF" } }, 
      fill: { fgColor: { rgb: "10B981" } } // Verde brillante FINPRO
    };

    // 2. Aplicamos estilos a las filas clave (basado en el orden de tu array)
    if(ws['A1']) ws['A1'].s = estiloOscuro; // Título "FINPRO STORE..."
    
    if(ws['A7']) ws['A7'].s = estiloVerde;  // "RESUMEN DE RESULTADOS"
    
    if(ws['A12']) ws['A12'].s = estiloVerde; // "DETALLE DE EQUIPOS VENDIDOS"
    
    if(ws['A13']) ws['A13'].s = estiloOscuro; // Cabecera "EQUIPO / ESPECIFICACIONES"
    if(ws['B13']) ws['B13'].s = estiloOscuro; // Cabecera "PRECIO"
    // --- FIN DE ESTILOS FINPRO ---
 ...
 const registrosParaTabla = useMemo(() => {
  ...
  const rendimientoUsuarios = useMemo(() => {
    const data = {
      ...
      'RAY': { cantidad: 0, total: 0, color: '#f97316' },
  ...
  const laptopsVendidasHistorico = useMemo(() => laptops.filter(l => (l.estado || '').toUpperCase().trim() === 'VENDIDO'), [laptops]);
