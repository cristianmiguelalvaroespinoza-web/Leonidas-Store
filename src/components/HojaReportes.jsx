...
{estaExpandido && mostrarFila && (
  <tr style={{ 
    backgroundColor: (modoEdicion && item.fireId && seleccionadosMasivos.has(item.fireId)) ? 'rgba(0, 255, 127, 0.18)' : 'transparent',
    boxShadow: (modoEdicion && item.fireId && seleccionadosMasivos.has(item.fireId)) ? 'inset 4px 0 0 0 #00ff7f' : 'none',
  ...
  {item.n_pedido || '-'}
        ...
        }, [datosFiltrados, diasExpandidos, modelosExpandidos, esVendedor, esAdmin2, laptops, modoEdicion, seleccionadosMasivos])}
        </table>
      </div>
    </div>
  );
