...
const toggleModelo = (clave) => {
 ...
 // --- LÓGICA DE FILTRADO ACTUALIZADA ---
  const datosFiltrados = (laptops || []).filter(d => {
    ...
    const caracteristicas = `${d.marca || ''} ${d.modelo || ''} ${d.serial || ''} ${d.procesador || ''} ${d.generacion || d.gen || ''} ${d.ram || ''} ${d.gpu || ''} ${d.almacenamiento || d.disco || ''} ${nombreVendedor} ${d.n_pedido || ''}`.toLowerCase();
  ...
  const exportarExcel = () => {
    ...
    ws['!pageSetup'] = {
      orientation: 'landscape', // Pone la hoja en horizontal, ideal para tablas anchas.
      fitToWidth: 1,            // Fuerza a la tabla a ocupar todo el ancho de la página.
      fitToHeight: 0,           // Permite que la tabla se extienda a lo largo de varias páginas si es necesario.
      scale: 85,                // Escala el contenido al 85%. Puedes ajustar este valor (ej. 90, 100).
    ...
    {/* --- CONTENEDOR FLEX PARA ALINEAR PESTAÑAS Y SELECTOR --- */}
      <div style={{ display: 'flex', alignItems: 'center', gap: '20px', marginBottom: '15px' }}>
        ...
        <div className={styles.estadoTabs} style={{ margin: 0 }}>
          ...
          <button onClick={() => setFiltroEstado('STOCK')} className={filtroEstado === 'STOCK' ? `${styles.active} ${styles.stock}` : ''}>🟢 STOCK ({laptops.filter(l => (l.estado || 'STOCK') === 'STOCK').length})</button>
          <button onClick={() => setFiltroEstado('VENDIDO')} className={filtroEstado === 'VENDIDO' ? `${styles.active} ${styles.vendido}` : ''}>🔴 VENDIDOS / SALIDA ({laptops.filter(l => l.estado === 'VENDIDO').length})</button>
          <button onClick={() => setFiltroEstado('PENDIENTE')} className={filtroEstado === 'PENDIENTE' ? `${styles.active} ${styles.pendiente}` : ''}>
            🟡 PENDIENTES POR RECIBIR ({laptops.filter(l => l.estado === 'PENDIENTE').length})
          </button>
        </div>
...
{estaExpandido && mostrarFila && (
  ...
  {item.n_pedido || '-'}
        ...
        }, [datosFiltrados, diasExpandidos, modelosExpandidos, esVendedor, esAdmin2, laptops])}
        </table>
      </div>
    </div>
  );
