...
const VentasView = ({
  ...
  const [tipoDocumento, setTipoDocumento] = useState("BOLETA"); // "BOLETA" o "NOTA" (Nota de Venta)
  ...
  const [fechaBoletaEditable, setFechaBoletaEditable] = useState(new Date().toLocaleDateString('es-PE'));
  ...
  const manejarVentaProxima = (laptop) => {
    ...
    setModoVenta(false);
    setTipoDocumento("BOLETA"); // Cada nueva venta inicia como Boleta de Venta
    setNombreCliente("");
    setDniCliente("");
    setOrigenVenta("LIMA");
    setDestinoVenta("");
    setFechaBoletaEditable(new Date().toLocaleDateString('es-PE'));
  };
  ...
  const descargarPDF = async () => {
    ...
    const selectorTipoDoc = document.querySelector(".selector-tipo-doc-no-imprimir");
    ...
    try {
      ...
      pdf.save(`${tipoDocumento === 'BOLETA' ? 'Boleta' : 'Nota_de_Venta'}_FINPRO_${nombreCliente.replace(/\s+/g, '_') || 'Cliente'}.pdf`);
    } catch (error) {
  ...
  const finalizarVentaYActualizar = async () => {
    ...
    try {
      ...
      const fechaFormateada = fechaBoletaEditable;
  ...
  return updateDoc(equipoRef, {
    ...
    dni: dniCliente.toUpperCase(), // DNI ahora es opcional
    ...
    numero_boleta: numeroBoletaCompleto
      ...
      setLaptopsSeleccionadas([]);
      setModoVenta(false);
      setTipoDocumento("BOLETA"); // Volvemos al modo Boleta por defecto
      setNombreCliente("");
      setDniCliente("");
    ...
    try {
      ...
      ventasOrdenadas.forEach((l, i) => {
        const rowData = {
          index: i + 1,
          fecha: l.fecha,
          vendedor: nombreVendedorVisible(l.responsable || l.vendedor),
          marca: l.marca,
          modelo: l.modelo,
    ...
    <div className="ventas-view-container fade-in">
        ...
        <div className="bar-multiventa mobile-stack" style={{ background: '#1e293b', padding: '15px', borderRadius: '8px', marginBottom: '10px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: '10px' }}>
            <button
              ...
              onClick={() => { setTipoDocumento("BOLETA"); setMostrarInforme(true); }}
      ...
      {mostrarInforme && (equipoDetalle || laptopsSeleccionadas.length > 0) && (
        <div className="overlay-informe-leonidas" style={{ alignItems: 'flex-start', paddingTop: '5vh' }}>
          <div className="contenedor-boleta-formal" style={{ backgroundColor: '#ffffff', borderRadius: '0px', border: '1px solid #ccc', boxShadow: 'none', color: '#000000' }}>
            ...
            {/* --- SELECTOR DE TIPO DE DOCUMENTO: BOLETA / NOTA DE VENTA (no aparece en el PDF) --- */}
            <div className="selector-tipo-doc-no-imprimir" style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', padding: '10px 0', marginBottom: '8px', borderBottom: '1px dashed #ccc' }}>
              <button
                onClick={() => setTipoDocumento(prev => (prev === 'BOLETA' ? 'NOTA' : 'BOLETA'))}
                style={{
                  display: 'flex',
                  alignItems: 'center',
                  gap: '8px',
                  background: tipoDocumento === 'BOLETA' ? '#1d4ed8' : '#047857',
                  color: '#fff',
                  border: 'none',
                  padding: '10px 20px',
                  borderRadius: '6px',
              ...
              >
                <FileText size={16} />
                {tipoDocumento === 'BOLETA' ? 'Cambiar a NOTA DE VENTA' : 'Cambiar a BOLETA DE VENTA'}
              </button>
            </div>
            ...
            <header className="header-boleta-formal">
              <div className="info-empresa">
                ...
                <p style={{color: '#333', fontSize: '0.8rem', margin: '5px 0 0 0'}}></p>
                <p style={{color: '#333', fontWeight: 'bold', margin: '5px 0 0 0'}}></p>
              </div>
              <div className="recuadro-tipo-doc" style={{ border: '1px solid #ccc', backgroundColor: '#f0f0f0', borderRadius: '0px', color: '#000' }}>
                <h2 style={{color: '#000'}}>{tipoDocumento === 'BOLETA' ? 'BOLETA DE VENTA' : 'NOTA DE VENTA'}</h2>
      ...
      <span style={{color: '#000'}}>LIMA</span>
                ...
                <div className="dato-cliente-fila">
                  ...
                  <span style={{color: '#000'}}>{fechaBoletaEditable}</span>
                </div>
                <div className="dato-cliente-fila">
                  <MapPin size={16} className="icono-dato" style={{ color: '#555' }} />
...
<table className="tabla-items-boleta" style={{ width: '100%', borderCollapse: 'collapse', marginTop: '10px' }}>
  ...
  <tfoot>
    {(() => {
      const listaVentaActual = laptopsSeleccionadas.length > 0 ? laptopsSeleccionadas : [equipoDetalle].filter(Boolean);
      const sumaTotalGeneral = listaVentaActual.reduce((acc, item) => acc + Number(item.precio || 0), 0);
            ...
            <div className="grupo-botones-boleta-no-imprimir">
              <div className="contenedor-confirmar-venta-formal">
                ...
                <input
                  type="text"
                  className="input-nombre-cliente-formal"
                  placeholder="Editar Fecha (DD/MM/AAAA)..."
                  value={fechaBoletaEditable}
                  onChange={(e) => setFechaBoletaEditable(e.target.value)}
                  style={{ background: '#fff', color: '#000', border: '1px solid #ccc', marginTop: '8px' }}
                />
                <input
                  type="text"
                  className="input-nombre-cliente-formal"
