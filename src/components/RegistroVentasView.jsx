...
const RegistroVentasView = ({
  ...
  useEffect(() => {
    if (editandoId && form.serial) {
      ...
      // Aseguramos que si estamos editando, el formulario mantenga los datos originales si existen
      setForm(prev => ({ 
        ...prev, 
        cantidad: 1,
        responsable: prev.responsable || form.responsable // Evita que se pierda el vendedor al cargar
      })); 
    } else if (!editandoId) {
      ...
      <form className={styles.formularioLeonidas} onSubmit={manejarEnvioLocal}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '15px', marginTop: '20px' }}>
              ...
              {usuarioLogueado?.rol === 'super_admin' ? (
                <>
                  ...
                  <select
                    ...
                    onChange={manejarCambio}
                    style={{ border: '1px solid #00ff7f', background: 'rgba(0, 255, 127, 0.05)', color: 'white' }}
                  >
            ...
            <div className={styles.campo}>
              ...
              <input
                ...
                onChange={manejarCambio}
                disabled={usuarioLogueado?.rol !== 'super_admin' && !editandoId}
        ...
        <footer className={styles.formFooterButtons} style={{ display: 'flex', gap: '15px', marginTop: '30px', borderTop: '1px solid rgba(255,255,255,0.1)', paddingTop: '20px' }}>
          ...
          <button
            ...
            style={{ flex: 1, background: '#94a3b8', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}
          >
            <X size={18} /> CANCELAR
          </button>
