...
// "Ray" se muestra como "Tienda Principal" | "Jefe"/"Leonidas" se muestra como "General"
const nombreVendedorVisible = (nombre) => {
  const limpio = String(nombre || '').toUpperCase().trim();
  if (limpio === 'RAY') return 'Tienda Principal';
  if (limpio === 'LEONIDAS' || limpio === 'JEFE' || limpio.includes('PIA')) return 'General';
  return nombre;
};
...
// CÓDIGO NUEVO (Ordena por Fecha Y Hora exacta):
  ...
  const manejarEnvioLocal = async (e) => {
    e.preventDefault();
    // La lógica para manejar múltiples seriales ahora está centralizada en `guardarLaptop` en App.jsx.
    // Esta función simplemente invoca el guardado. `guardarLaptop` usará el estado `listaSeriales`.
    const exito = await guardarLaptop(e);
    
    // Si el guardado fue exitoso (no se canceló o falló), reseteamos la lista de seriales para el próximo registro.
    if (exito !== false) {
      setListaSeriales([""]);
    }
  };
    ...
    if (e.key === 'Enter') {
      e.preventDefault();
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '15px', marginTop: '20px' }}>
            <div className={styles.campo}>
              {usuarioLogueado?.rol === 'super_admin' || usuarioLogueado?.nombre?.toUpperCase().includes('PIA') ? (
                <>
            ...
            {usuarioLogueado?.rol === 'super_admin' || (usuarioLogueado?.nombre?.toUpperCase() === 'RAY') || (tienePermiso && tienePermiso('ASIGNAR_PRECIOS')) ? (
                <div className={styles.campo}>
            ...
            <div className={styles.campo}>
                {usuarioLogueado?.rol === 'super_admin' || (usuarioLogueado?.nombre?.toUpperCase() === 'RAY') || (tienePermiso && tienePermiso('ASIGNAR_PRECIOS')) ? ( // Este es el campo PRECIO VENTA
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '15px', marginTop: '10px' }}>
            <div className={styles.campo}>
              ...
              <select name="estado" value={form.estado} onChange={manejarCambio}>
                {["STOCK", "VENDIDO", "PENDIENTE"].map(opt => <option key={opt} value={opt}>{opt}</option>)}
              </select>
            </div>
            <div className={styles.campo}>
                    ...
                    [...laptops, ...datosRapidos]
                      ...
                      <tr key={laptop.id || index} style={{ borderBottom: '1px solid #1e293b', backgroundColor: index % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.02)' }}>
                        ...
                        <td style={{ padding: '12px 8px', fontWeight: 'bold' }}>
                          {nombreVendedorVisible(typeof (laptop.responsable || laptop.usuario || laptop.vendedor) === 'object' 
