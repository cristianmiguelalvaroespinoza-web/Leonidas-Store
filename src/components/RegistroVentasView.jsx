...
const datosExcel = laptops.slice().reverse().map(laptop => ({
    ...
    "HORA": laptop.hora || '--:--',
    "USUARIO": laptop.usuario || laptop.responsable || '---', // Agregado
    "EQUIPO": laptop.equipo || '---',                        // Agregado
    "MARCA": laptop.marca || '---',                         // Agregado
    "MODELO": laptop.modelo || '---',                       // Agregado
    "ESPECIFICACIONES": `${laptop.procesador || ''} | ${laptop.ram || ''} | ${laptop.disco || ''} | ${laptop.gpu || ''}`,
  ...
  [...laptops]
    .sort((a, b) => {
      const convertirFecha = (f) => {
        if (!f || f === 'Sin fecha') return new Date(0);
        const [d, m, y] = f.split('/');
    ...
    <tr>
      <td colSpan="7" style={{ textAlign: 'center', padding: '20px', color: '#64748b' }}>No hay registros disponibles.</td>
    </tr>
  )}
</tbody>
